...
`;
  ...
  const args = [];
  const finalModel = model || MODELS.PRO;
  if (finalModel) { args.push(CLI.FLAGS.MODEL, finalModel); }
  if (sandbox) { args.push(CLI.FLAGS.SANDBOX); }

  // Use positional argument instead of -p flag
  ...
  try {
    ...
    if (errorMessage.includes(ERROR_MESSAGES.QUOTA_EXCEEDED) && model !== MODELS.FLASH_3 && model !== MODELS.FLASH) {
